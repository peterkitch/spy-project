...
- `--run-id` is optional; when omitted the adapter calls `generate_run_id("k6_mtf", "run_directory")` and produces a timestamp-based run id. A named run id is recommended for the operator campaign so the produced sidecar path is self-documenting in the controlled_compute manifest. The operator either replaces the `<OPTIONAL_NAMED_RUN_ID>` placeholder above with a real campaign run id or omits the `--run-id` line entirely; the placeholder MUST NOT be passed verbatim. The `<OPERATOR_FIXED_SEED>` placeholder is the same: substitute a real integer seed at run time (see Section 6 rng_seed policy).
...
- `expected_validation_sidecar` is NOT supplied. Two cases apply:
  - When `--run-id` is omitted, the adapter calls `generate_run_id("k6_mtf", "run_directory")` and the exact sidecar filename is not known up front, so an exact-path `expected_validation_sidecar` cannot be supplied.
  - When `--run-id <name>` is supplied, the sidecar path is predictable (`<REPO_ROOT>/project/output/validation/<name>/validation.json`).
  Discovery mode is preferred in both cases because controlled_compute snapshots `validation_sidecar_search_root` before the run and verifies that exactly one new `validation.json` was actually produced under `project/output/validation` after the run (per `_snapshot_validation_sidecars` / `_discover_new_validation_sidecars` in `project/controlled_compute.py`). That gives a stronger post-run guarantee than an exact-path expectation -- the campaign fails if the adapter wrote zero sidecars or more than one new sidecar -- and avoids a brittle hard-coded expected path in the campaign job-spec. This matches the StackBuilder onboarding precedent at `project/examples/controlled_compute/stackbuilder_onboarding_job_spec.json`.
...
- **Subsequent honest_validation_ledger output** at `<REPO_ROOT>/project/output/validation_ledger/honest_validation_ledger.json` and `honest_validation_ledger.md` when the operator separately runs `<PINNED_SPYPROJECT2_PYTHON> project/honest_validation_ledger.py --validation-root project/output/validation --output-dir project/output/validation_ledger` (per the module's documented CLI invocation; `<PINNED_SPYPROJECT2_PYTHON>` is the pinned `spyproject2` interpreter from CLAUDE.md Section 1). The ledger version is `validation_ledger_v1` (`honest_validation_ledger.py` `_LEDGER_VERSION`).
...
4. **Operator-run `honest_validation_ledger`** at `<PINNED_SPYPROJECT2_PYTHON> project/honest_validation_ledger.py --validation-root project/output/validation --output-dir project/output/validation_ledger`. Aggregates the K=6 MTF sidecar from item 3 (paired with the StackBuilder layer per PR #369 Option L4) into `honest_validation_ledger.json` and `honest_validation_ledger.md` under `output/validation_ledger/`.
