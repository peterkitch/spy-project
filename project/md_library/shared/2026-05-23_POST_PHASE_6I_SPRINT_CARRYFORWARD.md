...
- Item #3 stays OPEN because two operator decisions remain pending: k-patience alignment direction and sharpe-eps alignment direction. The allow-decreasing mechanical strand is resolved (see bullet above and PR #356). When the remaining two strands land, item #3 can transition OPEN -> RESOLVED per "How This Doc Is Used", citing the resolution date and final PR references.
